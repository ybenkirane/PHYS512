...
#Leapfrog Simulation: Periodic

atomos= Particles(partCount = 100000, dt =0.025 , periodicity = True)
...
for i in range(timeDomain):
    ...
    plt.savefig(f'Figures/Periodic/Leapfrog/{i:003}', dpi = 200)
    plt.show()
    plt.pause(0.0001)
...
plt.grid()
plt.legend()
plt.title("Periodic Leapfrog Energy Evolution")
plt.savefig("Figures/PeriodicLeapfrogEnergy.jpg")
plt.show()


with imageio.get_writer("Videos/" + "PeriodicLeapfrog" +".gif", mode="I") as writer:
    for filename in ["Figures/Periodic/Leapfrog/" + f'{tally:003}' + ".png" for tally in range(timeDomain)]:
...
#RK4 Simulation: Periodic

atomos= Particles(partCount = 100000, dt =0.025, periodicity = True)
atomos.uniformDistribution()
atomos.kernel = get_kernel(atomos.nMesh, atomos.soft, atomos.periodicity)
...
for i in range(timeDomain):
    ...
    plt.savefig(f'Figures/Periodic/RK4/{i:003}', dpi = 200)
    plt.show()
    plt.pause(0.0001)
...
plt.grid()
plt.legend()
plt.title("Periodic RK4 Energy Evolution")
plt.savefig("Figures/PeriodicRK4Energy.jpg")
plt.show()
    

with imageio.get_writer("Videos/" + "PeriodicRK4" +".gif", mode="I") as writer:
    for filename in ["Figures/Periodic/RK4/" + f'{tally:003}' + ".png" for tally in range(timeDomain)]:
        image = imageio.imread(filename)
        writer.append_data(image)
